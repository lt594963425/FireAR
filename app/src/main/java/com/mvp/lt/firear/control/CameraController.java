package com.mvp.lt.firear.control;
...
import static com.mvp.lt.firear.Constants.DEFAULT_CAMERA_DEGREE;
...
public class CameraController {
    ...
    public void initPreview(int previewWidth, int previewHeight, int rotation) {
        Log.e(TAG, "初始化摄像层预览状态:previewHeight"+previewWidth+",previewHeight"+previewHeight+","+rotation);
    ...
    private void initPreviewResolution() {
        Log.e(TAG, "宽：initPreviewResolution");
        if (mPreviewWidth <= 0 || mPreviewHeight <= 0) {// 检测是否使用默认值。
            ...
            mPreviewHeight = mPreviewHeight <= 0 ? display.getHeight()
                    : mPreviewHeight;
            Log.e(TAG, "宽：" + mPreviewWidth + "，高" + mPreviewHeight + "");
        }
    ...
    private Camera openCamera() {
        ...
        if (Build.VERSION.SDK_INT < 9 || null == mCamera) { // 低版本SDK或者上边初始化失败，直接打开摄像头
            Log.e(TAG,"低版本SDK或者上边初始化失败，直接打开摄像头");
            mCamera = Camera.open();
            setCameraDisplayOrientation(mCamera, null, true);
        }
     ...
     * @param useNewMethod 是否使用新的调整方向的方向
     */
    @SuppressWarnings("deprecation")
    private void setCameraDisplayOrientation(Camera camera,
        ...
        if (useNewMethod) {
            ...
            Log.e(TAG,"degrees"+degrees+",result"+degrees);
    ...
    public class CameraTextureListener implements SurfaceTextureListener {
        ...
        public void onSurfaceTextureAvailable(SurfaceTexture surface,
            ...
            initBestPreviewResolution(); //获得最佳预览分辨率。
            try {
    ...
    private void generateProjectionMatrix() {
        float ratio = (float) this.mPreviewWidth / this.mPreviewHeight;
        final int OFFSET = 0;
